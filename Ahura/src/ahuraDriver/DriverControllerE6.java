...
//		OpponentController1.relativeSpeedUpdater(noiseCan);
	...
	public void shutdown() {
		...
		if(myPara.getUseRecordedDangerZone().equals(UseRecordedDangerZones.WRITE) ||
                myPara.getUseRecordedDangerZone().equals(UseRecordedDangerZones.READWRITE)){
			if(myPara.getStage() != Stage.RACE) {
				//in the Race stage, the memory file is not generated because of the potential misinterpretation of
				// accidents vs bad driving.
				myPara.writeDangerzonesFile(getTrackName());
			}
		}
