...
import ahuraDriver.Controller.UseRecordedDangerZones;
...
//	String param = "distOvertakex: 0.287, distOvertakey: 7.0, opSteer: 0.4, d: 187.9075418536763, lambda2: 6.9, -absSlip: 1.0, x2: 95.30302118020252, c: 164.1158700288616, b: 90.47991440926471, -absMinSpeed: 3.0, a: 50.95124925780534, lambda1: 0.01956501913690144, e2: 5.382771708803534, y2: 6.230728885167587, x1: -0.2300585760015009, -absRange: 3.0, y1: 2.011735390446281";//default-slowest
    ...
    private UseRecordedDangerZones useRecordedDangerZones;
    ...
    public ParametersContainerE6(Stage stage, BlockerLevel level, UseRecordedDangerZones useRecordedDangerZones, String trackName) {
        setTrackName(trackName);
        setUseRecordedDangerZone(useRecordedDangerZones);
        setStage(stage);
        setLevel(level);
        readInitialization();
    }
    ...
    public void readInitialization() {
        ...
        if (useRecordedDangerZones.equals(UseRecordedDangerZones.READ) ||
                useRecordedDangerZones.equals(UseRecordedDangerZones.READWRITE)) {
            readDangerZoneFile(getTrackName());
    ...
    public void readDangerZoneFile(String trackName) {
        String fileToRead = trackName + ".txt";
        File f = new File(fileToRead);
        if (!f.exists()) {
            System.out.println("The memory file does not exist for the track " + trackName);
            if (useRecordedDangerZones.equals(UseRecordedDangerZones.READWRITE) && stage != Stage.RACE) {
                //in the Race stage, the memory file is not generated because of the potential misinterpretation of
                // accidents vs bad driving.
                System.out.println("A memory file is generated for the track " + trackName + " for future usages");
            }
            if (stage == Stage.RACE) {
                System.out.println("Note that, the memory file is NOT generated/updated in the Race stage, even if " +
                        "the option has been set to generate the memory file. Please change the stage to warm-up (0).");
            }
            return;
        }
        System.out.println("The memory file for the track " + trackName + " has been loaded");
        if (stage == Stage.RACE) {
            System.out.println("Note that, the memory file is NOT generated/updated in the Race stage, even if " +
                    "the option has been set to generate the memory file. Please change the stage to warm-up (0).");
        }
        ...
        try {
            while ((line = reader.readLine()) != null) {
                dangerousZonesList.add(new DangerZoneInfo(Double.parseDouble(line.split(" ")[0]), Integer.parseInt(line.split(" ")[1]), DangerzoneCause.valueOf(line.split(" ")[2])));
            }
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
...
//				System.out.println("Entering danger zone with severity " + i.y);
    ...
    public void writeDangerzonesFile(String trackName) {
//		String s = printOutResults(dist);

        String fileToSave = trackName + ".txt";
        File f = new File(fileToSave);
        FileWriter out = null;
        try {
        ...
        for (DangerZoneInfo info : dangerousZonesList) {
...
//		lambdaNew = Math.max(lambdaNew, 4.5);
    ...
    public UseRecordedDangerZones getUseRecordedDangerZone() {
        return useRecordedDangerZones;
    }

    public void setUseRecordedDangerZone(UseRecordedDangerZones useRecordedDangerZones) {
        this.useRecordedDangerZones = useRecordedDangerZones;
    }
